<template>
  <NavbarComponent />
  <div class="container overflow-hidden px-4 lg:py-8 lg:px-40 min-w-full">
    <GridComponent
      :gap="'gap-4'"
      :addClass="'overflow-hidden'"
      :col="'lg:grid-cols-2 grid-cols-1'"
    >
      <FlexComponent
        :gap="'gap-0'"
        :addClass="'lg:h-[500px] h-[300px] bg-slate-100 overflow-hidden'"
      >
        ...
        <div class="w-[80%] p-3 h-full overflow-hidden">
          <FlexComponent
            :justify="'center'"
            :align="'center'"
            :addClass="'w-full h-full overflow-hidden'"
          >
            <img class="h-full" :src="product.image" alt="" />
          </FlexComponent>
        </div>
      </FlexComponent>
      <FlexComponent :direction="'column'" :addClass="'lg:px-4'">
        <BraedCrumb
          :path="[
            { name: 'Home', url: '/' },
            { name: 'Product', url: '/product-details' },
            { name: 'Test', url: '/' },
          ]"
        />
        ...
        <FlexComponent :align="'center'">
          <span
            class="text-slate-400 text-sm uppercase rounded-md py-1 px-2 border"
            >xl</span
          >
          <span
            class="text-slate-400 text-sm uppercase rounded-md py-1 px-2 border"
        ...
        <FlexComponent :align="'start'" :direction="'column'">
          ...
          <FlexComponent>
            <span
              class="text-slate-400 text-sm uppercase rounded-full w-[20px] h-[20px]"
        ...
        <FlexComponent :addClass="'mt-4 w-full'">
          <ButtonComponent
            text="Add To Cart"
            :fontWeight="'light'"
            :rounded="'rounded-md'"
            ...
            :customClass="'bg-indigo-700 text-center lg:w-max w-1/2'"
          />
          <ButtonComponent
            :text="product.price"
            :styleButton="'outline'"
            ...
            :rounded="'rounded-md'"
            :fontSize="'0.8em'"
            :customClass="'lg:w-max text-center w-1/2'"
          />
        </FlexComponent>
  ...
  <div class="lg:mt-14 mt-6 px-4 lg:px-40 overflow-hidden">
    <SectionTitle :title="'Product Descriptions'" />
    <GridComponent :gap="'gap-0 lg:gap-4'" :addClass="'overflow-hidden'" :col="'lg:grid-cols-2 grid-cols-1'">
      <div class="w-full">
        <ul class="flex w-full justify-between">
          <li
            v-for="(tab, index) in tabs"
            :key="index"
            :class="[index == 0 ? 'pl-0 border-b-2' : '', `tab`]"
            class="py-2 px-3 border-slate-500 text-center"
            @click="handleTab(index, $event)"
          >
            {{ tab }}
          </li>
        </ul>
        <div v-if="tab1" class="py-4">
          ...
          <table class="mt-4 w-full border">
            <tr class="bg-slate-400 border-slate-500 border">
              <td class="p-4 text-sm border-slate-500 border">
                <strong class="block">Fabric</strong> Bio-washen cottom
              </td>
              <td class="p-4 text-sm border-slate-500 border">
                <strong class="block">Fabric</strong> Bio-washen cottom
              </td>
        ...
        <div v-if="tab2">Tab 2</div>
        <div v-if="tab3">Tab 3</div>
      </div>
      <FlexComponent :addClass="'p-4'" :justify="'center'" :align="'center'">
        <div class="w-[350px] h-[200px] rounded-md bg-slate-500"></div>
      </FlexComponent>
    </GridComponent>
  </div>
  <div class="lg:px-40 mt-4 px-4 lg:mt-8">
    <CardSlider
      :products="products"
      :wrap="true"
